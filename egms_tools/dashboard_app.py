...
def build_app(out_path, seed=0, heat_override=None, heat_timelapse=None):
    ...
    heat_override: optional dict {"png": data-uri, "bounds": [[s,w],[n,e]]}
        from REAL Landsat LST (see REAL_DATA.md). When given, the synthetic
        urban-heat surface is replaced by your real one; the city heat panel is
        dropped unless you also pass real city stats in the dict as "cities".
    heat_timelapse: optional dict {"frames":[...], "dates":[...], "bounds":[[s,w],[n,e]]}
        of yearly real LST frames; replaces the synthetic time-lapse.
    """
    ...
    if heat_override:                                   # REAL Landsat LST
